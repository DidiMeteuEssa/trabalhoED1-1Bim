...
void finalizar(int atendimentos, int totalTempoAtendimento)
{
	...
	gotoxy(12, 24);
	printf("Total de atendimentos: %d", atendimentos);
	gotoxy(12, 25);
	printf("Tempo medio por paciente: %.2lf ut\n", atendimentos ? (double)totalTempoAtendimento / atendimentos : 0);
}
	...
	else
	{
		...
		do
		{
			...
			if (emAtendimento.id == -1 && !Vazia(fila.cont))
			{
				...
				tempoPacienteAtual = rand() % 4 + 3;
				tempoRestante = tempoPacienteAtual;
			}
			...
			gotoxy(12, 25);
			printf("Pacientes na fila: %d | Pacientes lidos do arquivo: %d\n", fila.cont, lidos);
			gotoxy(12, 26);
		...
		} while ((!Vazia(fila.cont) || emAtendimento.id != -1) && !kbhit());
